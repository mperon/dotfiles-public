...
T_GIT_HTTP_URL=https://github.com/mperon/dotfiles-public.git
T_GIT_SSH_URL=git@github.com:mperon/dotfiles-public.git

[[ -z "$T_GIT_DIR" ]] && T_GIT_DIR=$HOME/.config/dotfiles
[[ -z "$T_WORK_TREE" ]] && T_WORK_TREE=$HOME
[[ -z "$T_FORCE" ]] && T_FORCE=
# public repository need to use HTTP
...
create_sshkey() {
    ...
    if [[ "$pk_count" -lt "1" ]]; then
        ...
        if [[ "${PIPESTATUS[0]}" -ne 0 ]]; then
           ...
           echo "You need to create manually later!"
           return 4
        fi
...
#before everything, check ssh things
...
if [[ ! -d "$T_GIT_DIR" ]]; then
    ...
    if [[ "${PIPESTATUS[0]}" -ne 0 ]]; then
        ...
        echo "   1) If you have internet connection"
        echo "   2) You can acesses following page:"
        echo "      $T_GIT_HTTP_URL"
        echo ""
        echo "Fix the problem an run this command again!"
        echo ""
        # just exits
        exit 4
    fi
...
if [[ ! -d "$T_GIT_DIR" ]]; then
    echo "An error ocurred when installing from git."
    echo "The $T_GIT_URL was not installed on $T_GIT_DIR "
