...
# GET MACBOOK SCREEN SIZE
#MAC_ARCH= MAC_MODEL= MAC_SIZE= MAC_YEAR=
source "${BASH_SOURCE%/*}/../mac.id.sh"

# Identify User Name
USER_NAME=$(id -F | cut -d" " -f2)
if [[ -z "$USER_NAME" ]]; then
    USER_NAME=$(id -F | cut -d" " -f1)
fi
if [[ -z "$USER_NAME" ]]; then
    __ask "Informe seu nome:" && USER_NAME=$E_ANSWER
fi
if [[ -n "$USER_NAME" ]]; then

    # Only first name (macbook)
    MODEL_NAME=$(echo 'MAC_MODEL' || cut -d" " -f1 | sed 's/ /-/g')
    COMPUTER_NAME="MacBook-${USER_NAME}-$MAC_SIZE"
else
    # first and second name
    MODEL_NAME=$(echo 'MAC_MODEL' | sed 's/ /-/g')
    COMPUTER_NAME="${MODEL_NAME}-${MAC_SIZE}"
fi
...
# its search on $HOME/.local/share/wallpapers for a os wallpaper
# wpath=$HOME/.local/share/wallpapers/$MY_OS.jpg
# corepath=/System/Library/CoreServices/
# default_bkg=/System/Library/PrivateFrameworks/SystemDesktopAppearance.framework/Resources/DefaultBackground.jpg
# defaultpath=$corepath/DefaultBackground.jpg
# if [[ -f $wpath ]]; then
#     [[ -f ~/Library/Application\ Support/Dock/desktoppicture.db ]] && rm -f ~/Library/Application Support/Dock/desktoppicture.db
#     sudo mv $defaultpath defaultpath.bkp
#     sudo mv $wpath $defaultpath
# fi
# to restore default background (uncomment these lines)
#[[ -f ~/Library/Application Support/Dock/desktoppicture.db ]] && rm -f ~/Library/Application Support/Dock/desktoppicture.db
#[[ -f $defaultpath ]] && sudo rm -f $defaultpath
#sudo ln -s $default_bkg $defaultpath
###############################################################################
# SSD-specific tweaks                                                         #
###############################################################################
...
# Possible values for PlistBuddy are none, date, name, kind, dateAdded, dateCreated etc.
/usr/libexec/PlistBuddy -c \
    "Set :StandardViewSettings:ExtendedListViewSettings:sortColumn name" \
    ~/Library/Preferences/com.apple.finder.plist
...
defaults write com.apple.Safari IncludeyalDebugMenu -bool true
